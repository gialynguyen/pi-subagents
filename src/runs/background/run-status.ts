				...
				if (step.runner?.type === "external-cli") {
					...
					if (runner) {
						...
						if (runner.safety) {
							...
							else if ("permissionMode" in runner.safety && "workspaceTrust" in runner.safety) lines.push(`  Safety: access=${runner.safety.access}, auth=${runner.safety.authentication}, permission=${runner.safety.permissionMode}, workspaceTrust=${runner.safety.workspaceTrust}, sessionReuse=${runner.safety.sessionReuse}`);
